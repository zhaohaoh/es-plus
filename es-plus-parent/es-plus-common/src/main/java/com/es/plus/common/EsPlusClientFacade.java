...
import com.es.plus.common.lock.EsReadWriteLock;
...
import com.es.plus.common.util.CollectionUtil;
...
public class EsPlusClientFacade   {
    ...
    public List<String> saveOrUpdateBatch(String type, Collection<?> esDataList,String... index) {
        List<String> failBulkItemResponses = new ArrayList<>();
        ...
        if (esDataList.size() > batchSize) {
            ...
            List<String> bulkItemResponses = esPlusClient.saveOrUpdateBatch( type, esDataList,index);
     ...
     */
    public List<String> saveBatch(String type, Collection<?> esDataList,String... index) {
        List<String> failBulkItemResponses = new ArrayList<>();
        if (CollectionUtils.isEmpty(esDataList)) {
            return failBulkItemResponses;
        }
        ...
        if (esDataList.size() > batchSize) {
            List<? extends Collection<?>> collections = CollectionUtil.splitList(esDataList, batchSize);
            collections.forEach(list -> {
                        List<String> bulkItemResponses = esPlusClient.saveBatch( type, list,index);
                        failBulkItemResponses.addAll(bulkItemResponses);
                    }
            );
        } else {
            List<String> bulkItemResponses = esPlusClient.saveBatch( type, esDataList,index);
            failBulkItemResponses.addAll(bulkItemResponses);
        }
        return failBulkItemResponses;
    }
     ...
     * @param index 索引
     */
    public List<String> updateBatch(String type, Collection<?> entityList,String... index) {
        List<String> failBulkItemResponses = new ArrayList<>();
        ...
        if (entityList.size() > batchSize) {
            ...
            collections.forEach(list -> {
                        List<String> bulkItemResponses = esPlusClient.updateBatch( type, list,index);
                        failBulkItemResponses.addAll(bulkItemResponses);
                    }
            );
        } else {
    ...
    public String explain(String sql) {
        return esPlusClient.explain(sql);
    }
    
    public String sql2Dsl(String sql) {
        return esPlusClient.sql2Dsl(sql,false);
    }
